...
/* parasoft-begin-suppress METRICS-36-3 Allow common functions to be called from more than 5 places, DRV-3823 */
...
/**
 * The maximum number of iterations during command response reading.
 */
#define CMD_RESPONSE_TIMEOUT 100U
/**
...
/**
 * The maximum number of bytes in reg
 */
# define I3C_REG_WIDTH_BYTES 4U

#ifndef I3C_GPI_CSRS_PER_APB_REG
#define I3C_GPI_CSRS_PER_APB_REG 4U
#endif

#ifndef I3C_GPO_CSRS_PER_APB_REG
#define I3C_GPO_CSRS_PER_APB_REG 4U
#endif

#ifndef I3C_APB_RO_CSRS_PER_APB_REG
#define I3C_APB_RO_CSRS_PER_APB_REG 4U
#endif

#ifndef I3C_APB_RW_CSRS_PER_APB_REG
#define I3C_APB_RW_CSRS_PER_APB_REG 4U
#endif

# define I3C_MAX_APB_RW_CSRS 4095U
# define I3C_MAX_APB_RO_CSRS 4095U
# define I3C_MAX_GPI_CSRS 4095U
# define I3C_MAX_GPO_CSRS 4095U
 ...
 * @return CDN_EIO if I3C Core did not enter idle state before timeout
 *  (WAIT_FOR_CORE_BUSY_TIMEOUT).
 */
static inline uint32_t wait_for_core_idle(I3C_PrivData *pd)
{
    uint32_t result = CDN_EIO;
...
/**
 * Returns true if manual command start is enabled,
 * @param[in] pd Pointer to I3C Core Driver's private data structure.
 */
static bool mcs_enabled(I3C_PrivData *pd)
{
    uint32_t reg_val;
    reg_val = CPS_REG_READ(&pd->regs_base->CTRL);
    return ((reg_val & MIPI__CTRL__MCS_EN_MASK) != 0U);
}
...
static void disable_mcs(I3C_PrivData *pd)
{
    uint32_t reg_val;
    reg_val = CPS_REG_READ(&pd->regs_base->CTRL);
    reg_val = CPS_FLD_CLEAR(MIPI__CTRL, MCS_EN, reg_val);
    CPS_REG_WRITE(&pd->regs_base->CTRL, reg_val);
}
/**
...
{
    return (uint8_t) ((((cfg->sclLowPeriod * (cfg->sysClk / 1000U)) / 1000000U)
...
static void update_device_info(I3C_PrivData *pd)
{
    ...
    for (i = 0; i < pd->max_devs; ++i) {
        ...
        rr0_val = CPS_REG_READ(rr0_reg);
        rr1_val = CPS_REG_READ(rr1_reg);
        rr2_val = CPS_REG_READ(rr2_reg);
        ...
        /* parasoft-begin-suppress MISRA2012-RULE-12_2-2 The right-hand operand of a shift operator shall lie */
        /*          in the range zero to one less than the width in bits of the essential type of the left-hand */
        /*          operand, DRV-5443 */
        /* RHS operand of shift will always be less than 32 */
 ...
 * @return CDN_EPROTO if transfer error was detected.
 */
...
/**
 *  Handler for Set group Address for SETGRPADDRESS CCC
 *
 *  This function is called before setgrpaddress command send.
 *  This function is updating Retaining Registers for
 *  device that that has been assigned with group Address.
 *
 *  @param pd[in] Pointer to driver's private data.
 *  @param error[in] Value of command response error field
 *  @param cmd[in] Pointer to command structure.
 */
static void ccc_set_grpaddr_complete_handler(I3C_PrivData *      pd,
                                             volatile I3C_cmd_t* cmd,
                                             uint8_t             error)
{
    I3C_DeviceDescriptor *dev;
    ...
    if (dev_idx != 0U) {
        ...
        if (error != 0U) {
            ...
            DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Failed to set GA. DA NACKed.\n");
            ...
            /*Update Group address for I3C device */
            ga = local_cmd->payload_sdr[0];             /* grp_addr + parity bit */

            /* find next empty devID */
            /* Look for device with da in all descriptors */
            for (i = 0; i < pd->max_devs; i++) {
                if (pd->devs[i].addr == (uint16_t)0) {
                    break;
                }
            }
            dev_rr0_reg = getAdvanceVolatileUint32Ptr(&(pd->regs_base->DEV_ID0_RR0), i * I3C_DEVS_OFF);
            reg_val = CPS_REG_READ(dev_rr0_reg);
            reg_val = CPS_FLD_WRITE(MIPI__DEV_ID0_RR0, DEV_ADDR, reg_val, ga);
            reg_val = CPS_FLD_WRITE(MIPI__DEV_ID0_RR0, IS_I3C, reg_val, true);
            CPS_REG_WRITE(dev_rr0_reg, reg_val);

            /* Set priv value as address of RR0 register */
            dev = &pd->devs[i];
            dev->priv = dev_rr0_reg;
            dev->addr = (uint16_t)ga >> 1;             /* remove parity bit */
            ...
            /*          in the range zero to one less than the width in bits of the essential type of the left-hand */
            /*          operand, DRV-5443 */
            /* RHS operand of shift will always be less than 32 because num_devs is checked against too big */
            /* values, also CONF_STATUS0.devs_num is encoded on 4 bits, it means that max value of this field is 15 */
            if ((dev_active & ((uint32_t)1U << i)) == 0U) {
...
uint32_t I3C_Probe(const I3C_Config *config, I3C_SysReq *sysReq)
{
     ...
     */
    if (I3C_ProbeSF(config, sysReq) != CDN_EOK) {
...
static uint32_t configure_devices(I3C_PrivData *pd, const I3C_Config *config){
    ...
    if ((pd->dev_role == I3C_DEV_ROLE_MASTER) &&
        (I3C_GetInstance()->configureDevices(pd, config->devs, config->numDevs) != CDN_EOK)) {
...
static void set_ibi_fifo_size(I3C_PrivData *pd, uint32_t conf_status1_val) {
    ...
    /*          in the range zero to one less than the width in bits of the essential type of the left-hand */
    /*          operand, DRV-5443 */
    /* RHS operand of shift will always be less than */
...
static void set_cmd_fifo_size(I3C_PrivData *pd, uint32_t conf_status1_val) {
    ...
    /*          in the range zero to one less than the width in bits of the essential type of the left-hand */
    /*          operand, DRV-5443 */
    /* RHS operand of shift will always be less than or equal to 5. CMD_MEM_DEPTH is encoded on 2-bit filed + 2U */
...
uint32_t I3C_ConfigurePrescalers(I3C_PrivData *pd, I3C_Config const *config)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_ConfigurePrescalersSF(pd, config) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    }
...
static void i3c_process_isr_tx_thr(I3C_PrivData *pd, uint32_t isr)
{
    ...
    if ((isr &  MIPI__MST_ISR__TX_THR_MASK) != 0U) {
        ...
        if (CLTraverse(&(pd->cmd_list), pd, fetch_command_write, pd->cmd_list.capacity) != 0U) {
...
static void i3c_process_isr_rx_thr(I3C_PrivData *pd, uint32_t isr)
{
    if ((isr &  MIPI__MST_ISR__RX_THR_MASK) != 0U) {
        ...
        if (CLTraverse(&(pd->cmd_list), pd, fetch_command_read, pd->cmd_list.capacity) != 0U) {
            DbgMsg(DBG_GEN_MSG, DBG_FYI, "Command list is empty.\n");
        }
                                         ...
                                         uint16_t            xfer_bytes)
{
    ...
    if (cmd != NULL) {
        ...
        else if (ccc == (uint8_t)I3C_CCC_SETGRPA_DC) {
            ccc_set_grpaddr_complete_handler(pd, cmd, error);
...
{
    uint32_t iterations_left = CMD_RESPONSE_TIMEOUT;
    uint32_t response;
...
static void i3cs_process_comp_isr(I3C_PrivData *pd, uint32_t isr)
{
    ...
    i3c_process_isr_callback(pd, isr, pd->callbacks.slaveIBIDone, MIPI__SLV_ISR__IBI_DONE_MASK);
    i3c_process_isr_callback(pd, isr, pd->callbacks.slaveSdrError, MIPI__SLV_ISR__ERROR_MASK);
    i3c_process_isr_callback(pd, isr, pd->callbacks.testMode, MIPI__SLV_ISR__TM_MASK);
    i3c_process_isr_callback(pd, isr, pd->callbacks.slaveMwlChange, MIPI__SLV_ISR__MWL_UP_MASK);
    i3c_process_isr_callback(pd, isr, pd->callbacks.slaveMrlChange, MIPI__SLV_ISR__MRL_UP_MASK);
    i3c_process_isr_callback(pd, isr, pd->callbacks.slaveResetDaa, MIPI__SLV_ISR__RSTDAA_MASK);
    i3c_process_isr_callback(pd, isr, pd->callbacks.slaveBusconUp, MIPI__SLV_ISR__BUSCON_UP_MASK);
    i3c_process_isr_callback(pd, isr, pd->callbacks.slaveFlushDone, MIPI__SLV_ISR__FLUSH_DONE_MASK);
}
...
uint32_t I3C_EnableCore(I3C_PrivData *pd)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_EnableCoreSF(pd) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
        reg_val = CPS_REG_READ(&pd->regs_base->CTRL);
...
uint32_t I3C_DisableCore(I3C_PrivData *pd)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_DisableCoreSF(pd) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
        reg_val = CPS_REG_READ(&pd->regs_base->CTRL);
...
uint32_t I3C_SetBusMode(I3C_PrivData *pd, I3C_BusMode mode)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_SetBusModeSF(pd, mode) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else if (I3C_GetInstance()->disableCore(pd) != CDN_EOK) {
...
uint32_t I3C_GetBusMode(I3C_PrivData *pd, I3C_BusMode *mode)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_GetBusModeSF(pd, mode) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
...
uint32_t I3C_SetBcr(I3C_PrivData *pd, uint8_t bcr, uint8_t devId)
{
    ...
    if (I3C_SetBcrSF(pd) != CDN_EOK) {
        ...
        reg_val = CPS_REG_READ(dev_bcr_reg);
        reg_val = CPS_FLD_WRITE(MIPI__DEV_ID0_RR2, BCR, reg_val, bcr);
        CPS_REG_WRITE(dev_bcr_reg, reg_val);
...
uint32_t I3C_SetDcr(I3C_PrivData *pd, uint8_t dcr, uint8_t devId)
{
    ...
    if (I3C_SetDcrSF(pd) != CDN_EOK) {
        ...
        reg_val = CPS_REG_READ(dev_dcr_reg);
        reg_val = CPS_FLD_WRITE(MIPI__DEV_ID0_RR2, DCR_LVR, reg_val, dcr);
        CPS_REG_WRITE(dev_dcr_reg, reg_val);
...
uint32_t I3C_SetPid(I3C_PrivData *pd, const uint8_t *pid, uint8_t devId)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_SetPidSF(pd, pid) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
        ...
        CPS_REG_WRITE(dev_pid_msb, pid_msb);
        reg_val = CPS_REG_READ(dev_pid_lsb);
        reg_val = CPS_FLD_WRITE(MIPI__DEV_ID0_RR2, PID_LSB, reg_val, pid_lsb);
        CPS_REG_WRITE(dev_pid_lsb, reg_val);
                             ...
                             uint8_t               index)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_ConfigureDeviceSF(pd, dev) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else if (core_enabled(pd)) {
        ...
        CPS_REG_WRITE(dev_rr0_reg, p_dev_arr[0]);
        CPS_REG_WRITE(dev_rr1_reg, p_dev_arr[1]);
        CPS_REG_WRITE(dev_rr2_reg, p_dev_arr[2]);
                                              ...
                                              I3C_DeviceDescriptor *devs, uint8_t num_devs)
{
    ...
    for (i = 0; i < num_devs; ++i) {
        ...
        if (I3C_GetInstance()->configureDevice(pd, dev, i) == CDN_EOK) {
            ...
            if (dev->legacyI2CDev == true) {
                ...
                /*          in the range zero to one less than the width in bits of the essential type of the left-hand */
                /*          operand, DRV-5443 */
                /* RHS operand of shift will always be less than 32 because num_devs is checked against too big values, also */
                /* CONF_STATUS0.devs_num is encoded on 4 bits, it means that max value of this field is 15 */
                if (dev->isActive) {
                    dev_active |= ((uint32_t)1U << i);
                } else {
                    dev_active &= ~((uint32_t)1U << i);
    ...
    if (I3C_GetInstance()->disableCore(pd) == CDN_EOK) {
                              ...
                              I3C_DeviceDescriptor *devs, uint8_t numDevs)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_ConfigureDevicesSF(pd, local_devs) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else if (core_enabled(pd)) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Core is enabled.\n");
...
uint32_t I3C_ClearRrOfDevice(I3C_PrivData* pd, uint8_t device_no)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_ClearRrOfDeviceSF(pd) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else if (!core_enabled(pd)) {
                                 ...
                                 const I3C_InterruptConfig* interruptConfig)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_ConfigureInterruptsSF(pd, interruptConfig) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else if (core_enabled(pd)) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Core is enabled.\n");
                                 ...
                                 const I3C_ThresholdConfig* thresholdConfig)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_ConfigureThresholdsSF(pd, thresholdConfig) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else if (core_enabled(pd)) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Core is enabled.\n");
...
{
    uint32_t address_of_ibi_issuer = 0U;
    uint32_t reg_val = 0U;

    /*
     * Validate parameters passed function
     */
    if (I3C_IbiGetAddressOfIssuerSF(pd) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
    } else {
                                 ...
                                 uint32_t                       num_sir_cfgs)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_IbiConfigureDevicesSF(pd, ibi_sir_cfg) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
                                   ...
                                   uint8_t                        da)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_IbiModifyDeviceConfigSF(pd, ibi_sir_cfg) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
...
uint32_t I3C_IbiGetData(I3C_PrivData *pd, uint8_t* buf, uint16_t payload_size)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_IbiGetDataSF(pd, buf) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
        bytes_xferred = 0U;
                                 ...
                                 I3C_HjDisableSet hj_disable_set)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_HjConfigureResponseSF(pd, hj_response, hj_disable_set) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
        reg_val = CPS_REG_READ(&pd->regs_base->CTRL);
...
uint32_t I3C_CmdMaxCount(const I3C_PrivData *pd)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdMaxCountSF(pd) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = 0U;
    } else {
...
uint32_t I3C_CmdClearAll(I3C_PrivData *pd)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdClearAllSF(pd) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
 ...
 * @param[in] cmd Pointer to I3C command structure.
 * @return CDN_EOK on success
 * @return CDN_EINVAL if Command is invalid for execution
 */
...
static uint32_t i3c_cmd_exec_execute_commands(I3C_PrivData *pd, volatile I3C_cmd_t* cmd)
{
     ...
     * Validate parameters passed function
     */
    if (validate_cmd_exec(pd, cmd) == CDN_EOK) {
        ...
        if ((cmd->is_wr_cmd == false) && (cmd->payload_size > pd->rx_fifo_size)) {
            pd->rx_cmd_in_progress = cmd;
        }
    }
...
/**
 * Internal Function.
 * Executes all queued commands utilizing MCS feature.
 */
static uint32_t i3c_cmd_exec(I3C_PrivData *pd)
{
    uint32_t result = CDN_EOK;
    size_t number_of_used_locations;
    bool mcs_en;

    mcs_en = mcs_enabled(pd);

    if (false == mcs_en) {
        /* Enable MCS if it's disabled. */
        (void) I3C_GetInstance()->enableMcs(pd);
    }

    number_of_used_locations = CLGetNumberOfUsedLocations(&(pd->cmd_list));
    result = CLTraverse(&(pd->cmd_list), pd, i3c_cmd_exec_execute, number_of_used_locations);
    /* Start executing commands in HW. */
    (void) I3C_GetInstance()->manualCommandStart(pd);

    if (false == mcs_en) {
        /* Disable MCS if it was disabled before. */
...
uint32_t I3C_CmdExecImmediate(I3C_PrivData *pd)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdExecImmediateSF(pd) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
        ...
        if (true == mcs_en) {
            /* Enable MCS if it was enabled before */
            (void) I3C_GetInstance()->enableMcs(pd);
        }
...
uint32_t I3C_DisableMcs(I3C_PrivData *pd)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_DisableMcsSF(pd) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    }
    else {
...
uint32_t I3C_EnableTc(I3C_PrivData *pd)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_EnableTcSF(pd) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    }
    else {
        reg_val = CPS_REG_READ(&pd->regs_base->CTRL);
        reg_val = CPS_FLD_SET(MIPI__CTRL, TC_EN, reg_val);
        CPS_REG_WRITE(&pd->regs_base->CTRL, reg_val);
        result = CDN_EOK;
    }
    return result;
...
uint32_t I3C_ManualCommandStart(I3C_PrivData *pd)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_ManualCommandStartSF(pd) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    }
    else {
                             ...
                             I3C_TransmitMode xmitMmode)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddPrivWriteSF(pd, csr_data, payload_data, xmitMmode) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
                               ...
                               uint8_t *payload, uint16_t payloadSize)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddPrivI2CReadSF(pd, payload) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
                                ...
                                uint8_t *payload, uint16_t payloadSize)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddPrivI2CWriteSF(pd, payload) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
        ...
        if (dev->legacyI2CDev) {
            ...
            if (cmd == NULL) {
                ...
                cmd->cmd |= (I3C_CMD_DIR_WRITE
                             ...
                             | i3c_cmd_get_payload_length(((uint64_t)payload_size / TX_FIFO_WORD_WIDTH))
                             | I3C_CMD_DDR_MODE);
                            ...
                            I3C_PayloadData *payload_data)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddDdrWriteSF(pd, hdrMode, payload_data) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
                           ...
                           I3C_PayloadData *payload_data)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddDdrReadSF(pd, hdrMode, payload_data) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else if ((!pd->use_fifos) && (payload_data->payloadSize > RX_FIFO_WORD_WIDTH)) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid payload size.\n");
                                  ...
                                  uint8_t devAddr, const I3C_TransmissionParameters *tx_params)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddSetSlaveEventsSF(pd, tx_params) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
                            ...
                            const I3C_TransmissionParameters *tx_params)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddResetDaaSF(pd, tx_params) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
...
uint32_t I3C_CmdAddEnterDaa(I3C_PrivData *pd, const I3C_TransmissionParameters *tx_params)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddEnterDaaSF(pd, tx_params) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
                                     ...
                                     const I3C_TransmissionParameters *tx_params)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddSetMaxWriteLengthSF(pd, tx_params) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
                                    ...
                                    uint8_t devAddr, const I3C_TransmissionParameters *tx_params)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddSetMaxReadLengthSF(pd, tx_params) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
                                    ...
                                    const I3C_TransmissionParameters *tx_params)
{
    ...
    if (I3C_CmdAddGetMaxReadLengthSF(pd,
                                     max_read_length, tx_params) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
                           ...
                           I3C_SlaveDescriptors *slaveDescs)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_GetSlavesListSF(pd, slaveDescs) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
                                    ...
                                    const I3C_TransmissionParameters *tx_params)
{
    ...
    if (I3C_CmdAddDefineSlavesListSF(pd,
        ...
        if (cmd == NULL) {
            ...
            cmd->cmd |= i3c_cmd_ccc((uint64_t)I3C_CCC_DEFTGTS_BC);
            cmd->cmd |= I3C_CMD_DIR_WRITE;
            /* count master in */
            /* +1 for slave number */
                                 ...
                                 const I3C_TransmissionParameters *tx_params)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddEnterTestModeSF(pd, tx_params) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
...
/**
 * Prepares CCC broadcast Command for all I3C Devices to set
 * bus context(i3c specification version that bus will use)
 * @param[in] pd Pointer to driver's private data object.
 * @param[in] context_buf Pointer to bus context data.
 * @param[in] length_size Number of bytes of context data
 * @param[in] tx_params Transmission parameters - command descriptor
 * @return CDN_EOK on success
 * @return CDN_EBUSY if Commands are being processed by the Core Driver
 * @return CDN_EINVAL if input parameters are invalid
 */
uint32_t I3C_CmdAddSetBuscon(I3C_PrivData *pd, uint8_t* context_buf, uint8_t length_size,
                             const I3C_TransmissionParameters *tx_params)
{
    uint32_t result = CDN_EOK;
    volatile I3C_cmd_t *cmd;
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddSetBusconSF(pd, context_buf, tx_params) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
        ...
        if (cmd == NULL) {
            ...
            cmd->cmd |= i3c_cmd_ccc((uint64_t)I3C_CCC_SETBUSCON_BC);
            cmd->cmd |= i3c_cmd_get_payload_length((uint64_t)length_size);
            cmd->payload_size = length_size;
            cmd->payload_sdr = (volatile uint8_t *)context_buf;
            cmd->is_wr_cmd = true;
                                ...
                                const I3C_TransmissionParameters* tx_params)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddEnterHdrModeSF(pd, mode, tx_params) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
...
/**
 * This function adds CCC for Dynamic Address Assignment using static address to
 * all target devices in broadcast mode.
 * @param[in] pd Pointer to driver's private data object.
 * @param[in] tx_params Transmission parameters - command descriptor
 * @return CDN_EOK on success
 * @return CDN_EBUSY if Commands are being processed by the Core Driver
...
uint32_t I3C_CmdAddSetAaSa(I3C_PrivData *pd, const I3C_TransmissionParameters *tx_params)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddSetAaSaSF(pd, tx_params) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
        ...
        if (cmd == NULL) {
            ...
            /* SETAASA broadcast message */
            cmd->cmd |= i3c_cmd_ccc((uint64_t)I3C_CCC_SETAASA_BC);
            cmd->payload_sdr = NULL;
            cmd->payload_ddr = NULL;
            cmd->payload_size = 0;
            cmd->bytes_fared = 0;
                            ...
                            const I3C_TransmissionParameters *tx_params)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdSetDaFromSaSF(pd, tx_params) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
                            ...
                            const I3C_TransmissionParameters *tx_params)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddSetNewDaSF(pd, tx_params) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
...
/**
 * Configure the next reset action for following reset pattern in I3C Device (or in all
   I3C Devices in case of a broadcast).
 * @param[in] pd Pointer to driver's private data object.
 * @param[in] definingByte Value of reset action applies on next reset pattern.
 * @param[in] devAddr Address of the I3C Device in case of Direct Message.
 * @param[in] tx_params Transmission parameters - command descriptors
 * @return CDN_EOK on success
 * @return CDN_EINVAL if input parameters are invalid
 * @return CDN_EBUSY if there is no space for new Command or payload
 */
uint32_t I3C_CmdAddSetRstAct(I3C_PrivData *pd, uint8_t definingByte,
                             uint8_t devAddr, const I3C_TransmissionParameters *tx_params)
{
    uint32_t result;
    volatile I3C_cmd_t *cmd;
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddSetRstActSF(pd, tx_params) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
        ...
        if (cmd == NULL) {
            ...
            if (tx_params->broadcast) {
                /* Broadcast command */
                cmd->cmd |= i3c_cmd_ccc((uint64_t)I3C_CCC_RSTACT_BC);
            } else {
                /* Direct command */
                cmd->cmd |= i3c_cmd_ccc((uint64_t)I3C_CCC_RSTACT_DC);
                cmd->cmd |= i3c_cmd_dev_addr(devAddr);
            }
            cmd->cmd |= I3C_CMD_DEFINE_BYTE_EN;
            cmd->cmd |= i3c_cmd_get_define_byte_value(definingByte);
/*            cmd->cmd |= i3c_cmd_get_payload_length(1U); */
            cmd->payload_size = 0U;
/*            cmd->payload_sdr = (volatile uint8_t *)&cmd->priv; */
/*            cmd->payload_sdr[0] = i3c_dev_addr_with_parity((uint8_t)devAddr); */
            cmd->is_wr_cmd = true;
...
/**
 * Exchange event timing information (for Timing Control Async Mode) with  I3C Device (or with all
   I3C Devices in case of a broadcast).
 * @param[in] pd Pointer to driver's private data object.
 * @param[in] SubCommand Byte Value of sub command code.
 * @param[in] devAddr Address of the I3C Device in case of Direct Message.
 * @param[in] tx_params Transmission parameters - command descriptors
 * @return CDN_EOK on success
 * @return CDN_EINVAL if input parameters are invalid
 * @return CDN_EBUSY if there is no space for new Command or payload
 */
uint32_t I3C_CmdAddSetXTime(I3C_PrivData *pd, uint8_t SubCommandByte,
                            uint8_t devAddr, const I3C_TransmissionParameters *tx_params)
{
    uint32_t result;
    volatile I3C_cmd_t *cmd;
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddSetXTimeSF(pd, tx_params) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
        ...
        if (cmd == NULL) {
            ...
            if (tx_params->broadcast) {
                /* Broadcast command */
                cmd->cmd |= i3c_cmd_ccc((uint64_t)I3C_CCC_SETXTIME_BC);
            } else {
                /* Direct command */
                cmd->cmd |= i3c_cmd_ccc((uint64_t)I3C_CCC_SETXTIME_DC);
                cmd->cmd |= i3c_cmd_dev_addr(devAddr);
            }
            cmd->cmd |= i3c_cmd_get_payload_length(1U);
            cmd->payload_size = 1;
            cmd->payload_sdr = (volatile uint8_t *)&cmd->priv;
            cmd->payload_sdr[0] = SubCommandByte;
            cmd->is_wr_cmd = true;
 ...
 * @param[in] pd Pointer to driver's private data object.
 * @param[in] address Group Address value.
 * @param[in] devAddr Address of the I3C Device to which Command will be sent.
 * @param[in] tx_params Transmission parameters - command descriptor
 * @return CDN_EOK on success
 * @return CDN_EBUSY if Commands are being processed by the Core Driver
 * @return CDN_EINVAL if input parameters are invalid
 */
uint32_t I3C_CmdAddSetGroupAddr(I3C_PrivData * pd, uint8_t grpAddr,
                                uint8_t devAddr, const I3C_TransmissionParameters *tx_params)
{
    uint32_t result = CDN_EOK;
    volatile I3C_cmd_t *cmd;
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddSetGroupAddrSF(pd, tx_params) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
        /* Get device index */
        dev_idx = get_dev_index(pd, devAddr);
        ...
        if (dev_idx != 0U) {
            ...
            if (cmd == NULL) {
                ...
                cmd->id = cmd_id;
                cmd->cmd = i3c_cmd_id((uint64_t) cmd_id);
                cmd->cmd |= i3c_cmd_ccc((uint64_t)I3C_CCC_SETGRPA_DC);
                cmd->cmd |= i3c_cmd_dev_addr(devAddr);
                cmd->cmd |= i3c_cmd_get_payload_length(1U);
                cmd->payload_size = 1;
                cmd->payload_sdr = (volatile uint8_t *)&cmd->priv;
                cmd->payload_sdr[0] = i3c_dev_addr_with_parity(grpAddr);
                cmd->bytes_fared = 0;
                cmd->in_cmd_fifo = false;
                cmd->is_wr_cmd = true;
...
/**
 * This function adds CCC for informing Secondary Masters mode details about
 * a particular Group,including the list of I3C Targets that are members of the Group.
 * @param[in] pd Pointer to driver's private data object.
 ...
 * @return CDN_EINVAL if input parameters are invalid
 */
uint32_t I3C_CmdAddDefineGroupList(I3C_PrivData *                    pd,
                                   ...
                                   const I3C_TransmissionParameters *tx_params)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddDefineGroupListSF(pd, groupDescs, tx_params) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
        ...
        if (cmd == NULL) {
            ...
            cmd->cmd |= i3c_cmd_ccc((uint64_t)I3C_CCC_DEFGRPA_BC);
            cmd->cmd |= I3C_CMD_DIR_WRITE;
            /* count master in */
            /* +1 for slave number */
            cmd->payload_size = (uint8_t)groupDescs->target_count + 3U;
            cmd->cmd |= i3c_cmd_get_payload_length((uint64_t)(cmd->payload_size));
            cmd->payload_sdr = (volatile uint8_t *)(groupDescs);
            cmd->is_wr_cmd = true;
...
/**
 * This function add CC to remove one or more I3C target devices from a Group by
 * resetting their assigned Group Address.
 * @param[in] pd Pointer to driver's private data object.
 * @param[in] grpAddr group address to reset.
 * @param[in] devAddr Address of the I3C Device in case of Direct Message.
 * @param[in] tx_params Transmission parameters - command descriptor
 * @return CDN_EOK on success
 * @return CDN_EBUSY if Commands are being processed by the Core Driver
 * @return CDN_EINVAL if input parameters are invalid
 */
uint32_t I3C_CmdAddResetGrpa(I3C_PrivData * pd,
                             uint8_t grpAddr, uint8_t devAddr,
                             const I3C_TransmissionParameters *tx_params)
{
    uint32_t result = CDN_EOK;
    volatile I3C_cmd_t *cmd;
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddResetGrpaSF(pd, tx_params) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
        ...
        if (cmd == NULL) {
            ...
            if (tx_params->broadcast) {
                /* Broadcast command */
                cmd->cmd |= i3c_cmd_ccc((uint64_t)I3C_CCC_RSTGRPA_BC);
                cmd->payload_sdr = NULL;
                cmd->payload_ddr = NULL;
                cmd->payload_size = 0;
                cmd->bytes_fared = 0;
                ...
                /* Direct command */
                cmd->cmd |= i3c_cmd_ccc((uint64_t)I3C_CCC_RSTGRPA_DC);
                cmd->cmd |= (grpAddr ? i3c_cmd_dev_addr(grpAddr) : i3c_cmd_dev_addr(devAddr));
                cmd->cmd |= I3C_CMD_DIR_WRITE;
                /* cmd->payload_size = (uint8_t)1U; */
                /* cmd->cmd |= i3c_cmd_get_payload_length(1U); */
                /* cmd->payload_sdr[0] = (grpAddr ? grpAddr : devAddr); */
                cmd->is_wr_cmd = true;
            }
            /* Start command if it is immediate */
            result = start_immediate_cmd(pd, tx_params->immediate);
                                    ...
                                    const I3C_TransmissionParameters *tx_params)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddGetProvisionalIdSF(pd, buff, tx_params) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
                          ...
                          const I3C_TransmissionParameters *tx_params)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddGetBcrSF(pd, bcr, tx_params) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
                          ...
                          const I3C_TransmissionParameters *tx_params)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddGetDcrSF(pd, dcr, tx_params) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
                             ...
                             const I3C_TransmissionParameters *tx_params)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddGetStatusSF(pd, status, tx_params) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
                             ...
                             const I3C_TransmissionParameters *tx_params)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddGetAccMstSF(pd, recData, tx_params) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
 ...
 * @return CDN_EINVAL if input parameters are invalid
 */
uint32_t I3C_CmdAddGetXTime(I3C_PrivData *                    pd,
                            ...
                            const I3C_TransmissionParameters *tx_params)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdAddGetXTimeSF(pd, tCamData, tx_params) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
        ...
        if (cmd == NULL) {
            ...
            cmd->cmd |= i3c_cmd_ccc((uint64_t)I3C_CCC_GETXTIME_DC);
            cmd->cmd |= i3c_cmd_dev_addr(devAddr);
            cmd->cmd |= I3C_CMD_DIR_READ;
            /* Payload size of TCAM0 command is 4 */
            cmd->cmd |= i3c_cmd_get_payload_length(4U);
            cmd->payload_size = 4;
            cmd->payload_sdr = (uint8_t *)tCamData;
            cmd->is_wr_cmd = false;
...
uint32_t I3C_CheckOperationMode(const I3C_PrivData* pd, I3C_OperationMode* opMode)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CheckOperationModeSF(pd, opMode) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
                                ...
                                const I3C_SlaveInterruptConfig *slaveInterruptConfig)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_SlaveModeConfigureSF(pd, slaveInterruptConfig) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
...
static void i3cs_process_config_interrupts(I3C_PrivData*                   pd,
    ...
    if (true == interrupt_config->mrDone) {
        /* enable mrDone interrupt */
        reg_val |= MIPI__SLV_IER__MR_DONE_MASK;
    }
    ...
    if (true == interrupt_config->defslvs) {
        /* enable defslvs interrupt */
        reg_val |= MIPI__SLV_IER__DEFSLVS_MASK;
    }

    if (true == interrupt_config->ibiDone) {
        /* enable ibiDone interrupt */
        reg_val |= MIPI__SLV_IER__IBI_DONE_MASK;
    }

    if (true == interrupt_config->ibid_thr) {
        /* enable ibid_thr interrupt */
        reg_val |= MIPI__SLV_IER__IBID_THR_MASK;
    }

    if (true == interrupt_config->mwl_up) {
        /* enable mwl_up interrupt */
        reg_val |= MIPI__SLV_IER__MWL_UP_MASK;
    }

    if (true == interrupt_config->mrl_up) {
        /* enable mrl_up interrupt */
        reg_val |= MIPI__SLV_IER__MRL_UP_MASK;
    }

    if (true == interrupt_config->resetDaa) {
        /* enable rstdaa interrupt */
        reg_val |= MIPI__SLV_IER__RSTDAA_MASK;
    }

    if (true == interrupt_config->buscon_up) {
        /* enable buscon_up interrupt */
        reg_val |= MIPI__SLV_IER__BUSCON_UP_MASK;
    }

    if (true == interrupt_config->flush_done) {
        /* enable flush_done interrupt */
        reg_val |= MIPI__SLV_IER__FLUSH_DONE_MASK;
    }
                                      ...
                                      const I3C_SlaveInterruptConfig* slaveInterruptConfig)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_ConfigureSlaveInterruptsSF(pd, slaveInterruptConfig) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else if (core_enabled(pd)) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Core is enabled.\n");
...
uint32_t I3C_SlaveModeReqSdrRead(I3C_PrivData *pd, uint8_t* buf, uint16_t num_bytes)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_SlaveModeReqSdrReadSF(pd, buf) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
        if (pd->rx_fifo_used > 0U) {
            possible_to_read = min(num_bytes, pd->rx_fifo_used);
        } else {
            possible_to_read = num_bytes;
        }
        while (bytes_xferred != possible_to_read) {

            /* Get number of bytes to read */
            bytes_to_read = min(possible_to_read - bytes_xferred,
                                RX_FIFO_WORD_WIDTH);
...
uint32_t I3C_SlaveModeReqSdrWrite(I3C_PrivData *pd, const uint8_t* buf, uint16_t num_bytes)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_SlaveModeReqSdrWriteSF(pd, buf) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
        possible_to_write = min(num_bytes,
                                (pd->tx_fifo_size - pd->tx_fifo_used));
        while (bytes_xferred < possible_to_write) {
            /* Get number of bytes to write */
            bytes_to_write = min(possible_to_write - bytes_xferred, TX_FIFO_WORD_WIDTH);
            ...
            for (i = 0; i < bytes_to_write; ++i) {
                ...
                /* RHS operand of shift will always be less than 32 */
                /* because bytes_to_wite will never exceed TX_FIFO_WORD_WIDTH, maximum is 8U * 3U */
                data |= ((uint32_t)buf[byte_iterator] << shift);
                /* parasoft-end-suppress MISRA2012-RULE-12_2-2 */
                byte_iterator++;
            }
            CPS_REG_WRITE(&pd->regs_base->TX_FIFO, data);
            bytes_xferred += bytes_to_write;
        }
        /* Update payload length after slave fifo write */
        reg_val = CPS_REG_READ(&pd->regs_base->SLV_CTRL);
        reg_val = CPS_FLD_WRITE(MIPI__SLV_CTRL, PR_PL, reg_val, bytes_xferred);
        CPS_REG_WRITE(&(pd->regs_base->SLV_CTRL), reg_val);
        result = CDN_EOK;
    }
...
uint32_t I3C_SlaveModeReqDdrRead(I3C_PrivData *pd, uint16_t* buf, uint16_t num_words)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_SlaveModeReqDdrReadSF(pd, buf) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
        /* START: Code duplicated from i3cs, blocked by DRV-3078 */
                                  ...
                                  uint16_t        num_words_in)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_SlaveModeReqDdrWriteSF(pd, buf) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
        /* START: Code duplicated from i3cs, blocked by DRV-3078 */
...
uint32_t I3C_SlaveModeRequestHotJoin(I3C_PrivData *pd)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_SlaveModeRequestHotJoinSF(pd) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
        /* START: Code duplicated from i3cs, blocked by DRV-3078 */
...
uint32_t I3C_SlaveModeMastershipReq(I3C_PrivData *pd)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_SlaveModeMastershipReqSF(pd) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
        reg_val = CPS_REG_READ(&pd->regs_base->CTRL);
 ...
 * @param[in] pd Pointer to driver's private data object.
 * @param[in] tCam0Event value assigned to TCAM0_EVENT
 * @param[in] ibiTCam0 indicate IBI request is for TCAM0 or regular
 * @return CDN_EOK on success
 * @return CDN_EIO if operation failed
 * @return CDN_EINVAL if input parameters are invalid
 */
uint32_t I3C_SlaveModeRequestIbi(I3C_PrivData * pd, uint8_t tCam0Event, bool ibiTCam0)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_SlaveModeRequestIbiSF(pd) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {

        /*
         * trigger IBI request only if taget has valida DA,IBI.DIS is low,
         * application wirtes payload to IBI_DATA_FIFO.
         */

        status1_reg_val = CPS_REG_READ(&pd->regs_base->SLV_STATUS1);
        if (CPS_FLD_READ(MIPI__SLV_STATUS1, HAS_DA, status1_reg_val) != 0U) {
            if (CPS_FLD_READ(MIPI__SLV_STATUS1, IBI_DIS, status1_reg_val) == 0U) {
                /* Time Control Async Mode 0 IBI */
                if (ibiTCam0 == true) {
                    if (CPS_FLD_READ(MIPI__SLV_STATUS1, IBI_TCAM0_DIS, status1_reg_val) == 0U) {
                        reg_val = CPS_REG_READ(&pd->regs_base->SLV_IBI_CTRL);
                        reg_val = CPS_FLD_SET(MIPI__SLV_IBI_CTRL, IBI_SEL, reg_val);
                        reg_val = CPS_FLD_WRITE(MIPI__SLV_IBI_CTRL, IBI_TCAM0_EVENT, reg_val, tCam0Event);
                        reg_val = CPS_FLD_SET(MIPI__SLV_IBI_CTRL, IBI_INIT, reg_val);
                        CPS_REG_WRITE(&pd->regs_base->SLV_IBI_CTRL, reg_val);
                        result = CDN_EOK;
...
/**
 * read reg value using GPI interface
 * @param[in] pd Pointer to driver's private data object.
 * @param[in] idx register index.
 * @return [out] result reg value
 */
static uint8_t read_gpi(const I3C_PrivData *pd, uint16_t idx)
{
    uint8_t result;

#if I3C_GPI_CSRS_PER_APB_REG <= 4
    uint16_t reg_idx;
    uint16_t reg_off;
    uint32_t reg_val;
     ...
     */
    assert(pd);
    assert(idx < I3C_MAX_GPI_CSRS);
    /* find out register and byte offset using arithmatic logic */
    reg_idx = idx / (uint16_t)I3C_GPI_CSRS_PER_APB_REG;
    reg_off = idx % (uint16_t)I3C_GPI_CSRS_PER_APB_REG;
    reg_val = CPS_REG_READ(getAdvanceVolatileUint32Ptr(&(pd->regs_base->GPIR_WORD0), reg_idx));

    const uint16_t shift = reg_off * 8U;
    /* parasoft-begin-suppress MISRA2012-RULE-12_2-2 The right-hand operand of a shift operator shall lie */
    /*          in the range zero to one less than the width in bits of the essential type of the left-hand */
    /*          operand, DRV-5443 */
    result = (uint8_t)(reg_val >> shift);
    /* parasoft-end-suppress MISRA2012-RULE-12_2-2 */
#else
    result = 0U;
#endif
...
/**
 * read reg value using GPO interface
 * @param[in] pd Pointer to driver's private data object.
 * @param[in] idx register index.
 * @return [out] result reg value
 */
static uint8_t read_gpo(const I3C_PrivData *pd, uint16_t idx)
{
    uint8_t result;

#if I3C_GPO_CSRS_PER_APB_REG <= 4
    uint16_t reg_idx;
    uint16_t reg_off;
    uint32_t reg_val;
     ...
     */
    assert(pd);
    assert(idx < I3C_MAX_GPO_CSRS);
    /* find out register and byte offset using arithmatic logic */
    reg_idx = idx / (uint16_t)I3C_GPO_CSRS_PER_APB_REG;
    reg_off = idx % (uint16_t)I3C_GPO_CSRS_PER_APB_REG;
    reg_val = CPS_REG_READ(getAdvanceVolatileUint32Ptr(&(pd->regs_base->GPOR_WORD0), reg_idx));
...
/**
 * read reg value using APB(Read-Only) interface
 * @param[in] pd Pointer to driver's private data object.
 * @param[in] idx register index.
 * @return [out] result reg value
 */

static uint8_t read_apb_ro(const I3C_PrivData *pd, uint16_t idx)
{
    uint8_t result;
#if I3C_APB_RO_CSRS_PER_APB_REG <= 4
    uint16_t reg_idx;
    uint16_t reg_off;
    uint32_t reg_val;
     ...
     */
    assert(pd);
    assert(idx < I3C_MAX_APB_RO_CSRS);
    /* find out register and byte offset using arithmatic logic */
    reg_idx = idx / (uint16_t)I3C_APB_RO_CSRS_PER_APB_REG;
    reg_off = idx % (uint16_t)I3C_APB_RO_CSRS_PER_APB_REG;
    reg_val = CPS_REG_READ(getAdvanceVolatileUint32Ptr(&(pd->regs_base->GPIR_WORD0), reg_idx));
...
/**
 * read reg value using APB interface
 * @param[in] pd Pointer to driver's private data object.
 * @param[in] idx register index.
 * @return [out] result reg value
 */
static uint8_t read_apb_rw(I3C_PrivData *pd, uint16_t idx)
{
    uint8_t result;
#if I3C_APB_RW_CSRS_PER_APB_REG <= 4
    uint16_t reg_idx;
    uint16_t reg_off;
    uint32_t reg_val;
     ...
     */
    assert(pd);
    assert(idx < I3C_MAX_APB_RW_CSRS);
    /* find out register and byte offset using arithmatic logic */
    reg_idx = idx / (uint16_t)I3C_APB_RW_CSRS_PER_APB_REG;
    reg_off = idx % (uint16_t)I3C_APB_RW_CSRS_PER_APB_REG;
    reg_val = CPS_REG_READ(getAdvanceVolatileUint32Ptr(&(pd->regs_base->GPOR_WORD0), reg_idx));
...
/**
 * write reg value using APB interface
 * @param[in] pd Pointer to driver's private data object.
 * @param[in] idx register index.
 * @return [out] result reg value
 */
static void write_apb_rw(I3C_PrivData *pd,
                         uint16_t idx, uint8_t val)
{
    uint16_t reg_idx;
    uint16_t reg_off;
    uint32_t reg_val;
    ...
    uint16_t shift = reg_off * 8U;

#if I3C_APB_RW_CSRS_PER_APB_REG <= 4
    /* parasoft-begin-suppress MISRA2012-RULE-12_2-2 The right-hand operand of a shift operator shall lie */
    /*          in the range zero to one less than the width in bits of the essential type of the left-hand */
    /*          operand, DRV-5443 */
    reg_val &= ~((uint32_t)UINT8_MAX << shift);
    reg_val |= ((uint32_t)val << shift);
    /* parasoft-end-suppress MISRA2012-RULE-12_2-2 */
#else
    reg_val = 0U;
#endif
    ...
    CPS_REG_WRITE(getAdvanceVolatileUint32Ptr(&(pd->regs_base->GPOR_WORD0), reg_idx), reg_val);
}

/**
 * subroutine for IBI payload read
 * @param[in] pd Pointer to driver's private data object.
 * @param[in] buf Pointer to read memory.
 * @param[in] numbytes number of bytes to read.
 */
static void read_ibi_pl(I3C_PrivData *pd,
                        uint8_t *buf, uint8_t numbytes)
{
    uint8_t i;
    uint32_t reg_val;
    ...
    const uint8_t rest = (numbytes % 4U);

    uint8_t offset = 0U;
    /*read data fom fifo register and update in local buffer*/
    /* keep updating counter */

    for (i = 0; i < (numbytes / I3C_REG_WIDTH_BYTES); ++i) {
        reg_val = CPS_REG_READ(&(pd->regs_base->IBI_DATA_FIFO));
        buf[offset + 3U] = (uint8_t)(reg_val >> 24);
        buf[offset + 2U] = (uint8_t)(reg_val >> 16);
        buf[offset + 1U] = (uint8_t)(reg_val >> 8);
        buf[offset + 0U] = (uint8_t)(reg_val);

        offset += I3C_REG_WIDTH_BYTES;
    }
    /* If the remaining data is less than a word width, we used following
     * logic to fill the remaining bytes to buf by shifting byte data from reg_val */
    if (rest != 0U) {
        reg_val = CPS_REG_READ(&(pd->regs_base->IBI_DATA_FIFO));
...
/**
 * subroutine for IBI payload write
 * @param[in] pd Pointer to driver's private data object.
 * @param[in] buf Pointer to write memory.
 * @param[in] numbytes number of bytes to write.
 */
static void write_last_byte(I3C_PrivData * pd, uint8_t const * buf, uint32_t *reg_val, uint8_t offset, uint8_t rest)
{
    /* The below logic is implement if the numbytes are not aligned to 4*/
    /* It writes last byte from TX buffer to IBI data fifo*/
    if ((rest == 3U) || (rest == 2U) || (rest == 1U)) {
        *reg_val |= (uint32_t)buf[offset + 0U];
        CPS_REG_WRITE(&(pd->regs_base->IBI_DATA_FIFO), *reg_val);
    }
}
 ...
 * @param[in] numbytes number of bytes to write.
 */
static void write_ibi_pl(I3C_PrivData * pd, uint8_t const * buf, uint8_t numbytes)
{
    uint8_t i;
    uint32_t reg_val;
    ...
    uint8_t offset = 0U;
    /* write data from local buffer to daata fifo and update count*/
    for (i = 0; i < (numbytes / I3C_REG_WIDTH_BYTES); ++i) {
        reg_val = (uint32_t)buf[offset + 0U]
    ...
    uint8_t rest = numbytes % I3C_REG_WIDTH_BYTES;
    /* The below logic is implement if the numbytes are not aligned to 4*/
    /* It writes remaining bytes partially into IBI data fifo */
    if (rest == 3U) {
        reg_val |= (uint32_t)buf[offset + 2U] << 16;
    }

    if ((rest == 3U) || (rest == 2U)) {
        reg_val |= (uint32_t)buf[offset + 1U] << 8;
    }

    /* This function is split in arbitrary place, because there was no other way to reduce VOCF */
    write_last_byte(pd, buf, &reg_val, offset, rest);

    /* Set IBI payload size appropriately */
    reg_val = CPS_REG_READ(&pd->regs_base->SLV_IBI_CTRL);
    reg_val = CPS_FLD_WRITE(MIPI__SLV_IBI_CTRL, IBI_PL, reg_val, numbytes);
    CPS_REG_WRITE(&(pd->regs_base->SLV_IBI_CTRL), reg_val);
}

/**
 * Write IBI payload before requesting IBI when operate in Slave mode.
 * @param[in] pd Pointer to driver's private data object.
...
uint32_t I3C_SlaveModeWriteIbiPayload(I3C_PrivData* pd, const uint8_t *buf, uint8_t numbytes)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_SlaveModeWriteIbiPayloadSF(pd, buf) != CDN_EOK) {
        result = CDN_EINVAL;
    } else {
        write_ibi_pl(pd, buf, numbytes);
        result = CDN_EOK;
    }
...
/**
 * Read IBI payload when operate in Slave mode.
 * @param[in] pd Pointer to driver's private data object.
 * @param[in] buf pointer to data buffer
 * @param[in] numbytes Number of bytes to read
 * @return CDN_EOK on success
 * @return CDN_EIO if operation failed
 * @return CDN_EINVAL if input parameters are invalid
 * @return CDN_ENOTSUP if feature is not supported
 */
uint32_t I3C_SlaveModeReadIbiPayload(I3C_PrivData* pd, uint8_t *buf, uint8_t numbytes)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_SlaveModeReadIbiPayloadSF(pd, buf) != CDN_EOK) {
        result = CDN_EINVAL;
    } else {
        read_ibi_pl(pd, local_buf, numbytes);
        result = CDN_EOK;
    }
...
uint32_t I3C_SlaveModeReqFlowControl(I3C_PrivData *pd, I3C_FlowControlMode mode)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_SlaveModeReqFlowControlSF(pd, mode) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
        reg_val = CPS_REG_READ(&pd->regs_base->SLV_STATUS1);
        /* Compare MACK mode and update register accodingly */
        /* For write operation in target mode, disable read nack and vice versa */
        if (mode == (I3C_FlowControlMode) I3C_NACK_NXT_PW_DISABLE) {
            reg_val = CPS_FLD_WRITE(MIPI__SLV_STATUS1, NACK_NXT_PW, reg_val, 1);
        }
        else {
            /* If the mode is not WRITE NACK disable, it is READ NACK disable as default */
            reg_val = CPS_FLD_WRITE(MIPI__SLV_STATUS1, NACK_NXT_PR, reg_val, 1);
        }

        CPS_REG_WRITE(&pd->regs_base->SLV_STATUS1, reg_val);
    }
...
/**
 * Read User purpose registers with APB (RO_access) when operate in Slave mode.
 * @param[in] pd Pointer to driver's private data object.
 * @param[in] idx register Id
 * @param[in] val Pointer to value object
 * @return CDN_EOK on success
 * @return CDN_EIO if operation failed
 * @return CDN_EINVAL if input parameters are invalid
 * @return CDN_ENOTSUP if feature is not supported
 */
uint32_t I3C_SlaveModeReadApbRo(I3C_PrivData* pd,
                                uint16_t idx, uint8_t *val)
{
    uint32_t result;
     ...
     * Validate parameters passed function
     */
    if (I3C_SlaveModeReadApbRoSF(pd, val) != CDN_EOK) {
        result = CDN_EINVAL;
    } else if (pd->num_apb_ro_csrs == 0U) {
        result = CDN_ENOTSUP;
    } else if (idx >= pd->num_apb_ro_csrs) {
        result = CDN_EINVAL;
    } else {
        /* Subroutine function to read */
        *val = read_apb_ro(pd, idx);
        result = CDN_EOK;
    }
...
/**
 * Read User purpose registers with APB (RW_access) when operate in Slave mode.
 * @param[in] pd Pointer to driver's private data object.
 * @param[in] idx register Id
 * @param[in] val Pointer to value object
 * @return CDN_EOK on success
 ...
 * @return CDN_ENOTSUP if feature is not supported
 */
uint32_t I3C_SlaveModeReadApbRw(I3C_PrivData* pd,
                                uint16_t idx, uint8_t *val)
{
    uint32_t result;
     ...
     * Validate parameters passed function
     */
    if (I3C_SlaveModeReadApbRwSF(pd, val) != CDN_EOK) {
        result = CDN_EINVAL;
    } else if (pd->num_apb_rw_csrs == 0U) {
        result = CDN_ENOTSUP;
    } else if (idx >= pd->num_apb_rw_csrs) {
        result = CDN_EINVAL;
    } else {
        /* Subroutine function to read */
        *val = read_apb_rw(pd, idx);
        result = CDN_EOK;
    }
...
/**
 * Write purpose registers with APB (RW_access) when operate in Slave mode.
 * @param[in] pd Pointer to driver's private data object.
 * @param[in] idx register Id
 * @param[in] val Pointer to value object
 * @return CDN_EOK on success
 ...
 * @return CDN_ENOTSUP if feature is not supported
 */
uint32_t I3C_SlaveModeWriteApbRw(I3C_PrivData* pd,
                                 uint16_t idx, uint8_t val)
{
    uint32_t result;
     ...
     * Validate parameters passed function
     */
    if (I3C_SlaveModeWriteApbRwSF(pd) != CDN_EOK) {
        result = CDN_EINVAL;
    } else if (pd->num_apb_rw_csrs == 0U) {
        result = CDN_ENOTSUP;
    } else if (idx >= pd->num_apb_rw_csrs) {
        ...
        /* Subroutine function to write */
        write_apb_rw(pd, idx, val);
        result = CDN_EOK;
    }
...
/**
 * Read User purpose registers with GPO Interface when operate in Slave mode.
 * @param[in] pd Pointer to driver's private data object.
 * @param[in] idx register Id
 * @param[in] val Pointer to value object
 * @return CDN_EOK on success
 ...
 * @return CDN_ENOTSUP if feature is not supported
 */
uint32_t I3C_SlaveModeReadGpo(I3C_PrivData* pd,
                              uint16_t idx, uint8_t *val)
{
    uint32_t result;
     ...
     * Validate parameters passed function
     */
    if (I3C_SlaveModeReadGpoSF(pd, val) != CDN_EOK) {
        result = CDN_EINVAL;
    } else if (pd->num_gpo_csrs == 0U) {
        result = CDN_ENOTSUP;
    } else if (idx >= pd->num_gpo_csrs) {
        result = CDN_EINVAL;
    } else {
        /* Subroutine function to read */
        *val = read_gpo(pd, idx);
...
/**
 * Read User purpose registers with GPI Interface when operate in Slave mode.
 * @param[in] pd Pointer to driver's private data object.
 * @param[in] idx register Id
 * @param[in] val Pointer to value object
 * @return CDN_EOK on success
 ...
 * @return CDN_ENOTSUP if feature is not supported
 */
uint32_t I3C_SlaveModeReadGpi(I3C_PrivData* pd,
                              uint16_t idx, uint8_t *val)
{
    uint32_t result;
     ...
     * Validate parameters passed function
     */
    if (I3C_SlaveModeReadGpiSF(pd, val) != CDN_EOK) {
        result = CDN_EINVAL;
    } else if (pd->num_gpi_csrs == 0U) {
        result = CDN_ENOTSUP;
    } else if (idx >= pd->num_gpi_csrs) {
        result = CDN_EINVAL;
    } else {
        /* Subroutine function to read */
        *val = read_gpi(pd, idx);
...
/**
 * Read MWL when operate in Slave mode.
 * @param[in] pd Pointer to driver's private data object.
 * @param[in] pointer to length variable
 * @return CDN_EOK on success
 * @return CDN_EINVAL if input parameters are invalid
 */
uint32_t I3C_SlaveModeReadMWL(I3C_PrivData* pd, uint16_t *maxWriteLength)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_SlaveModeReadMWLSF(pd, maxWriteLength) != CDN_EOK) {
        result = CDN_EINVAL;
    } else {
        reg_val = CPS_REG_READ(&pd->regs_base->SLV_STATUS3);
        *maxWriteLength = (uint16_t)CPS_FLD_READ(MIPI__SLV_STATUS3, MWL, reg_val);
        result = CDN_EOK;
    }
...
/**
 * Read MRL when operate in Slave mode.
 * @param[in] pd Pointer to driver's private data object.
 * @param[in] pointer to MRL variable
 * @param[in] pointer to ibi payload_size variable
 * @return CDN_EOK on success
 * @return CDN_EINVAL if input parameters are invalid
 */
uint32_t I3C_SlaveModeReadMRL(I3C_PrivData* pd, uint16_t *maxReadLength, uint8_t* ibiPayloadSize)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_SlaveModeReadMRLSF(pd, maxReadLength, ibiPayloadSize) != CDN_EOK) {
        result = CDN_EINVAL;
    } else {
        reg_val = CPS_REG_READ(&pd->regs_base->SLV_STATUS2);
        *maxReadLength = (uint16_t)(CPS_FLD_READ(MIPI__SLV_STATUS2, MRL, reg_val) >> 8);
        *ibiPayloadSize = (uint8_t)CPS_FLD_READ(MIPI__SLV_STATUS2, MRL, reg_val);
        result = CDN_EOK;
    }
 ...
 * @param[in] pd Pointer to driver's private data object.
 * @param[in] pointer to tx fill level variable
 * @param[in] pointer to rx fill level variable
 * @return CDN_EOK on success
 * @return CDN_EINVAL if input parameters are invalid
 */
uint32_t I3C_GetFifoFillLvl(I3C_PrivData* pd, uint16_t* fillLvl_tx, uint16_t* fillLvl_rx)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_GetFifoFillLvlSF(pd, fillLvl_tx, fillLvl_rx) != CDN_EOK) {
        result = CDN_EINVAL;
    } else {
        reg_val = CPS_REG_READ(&pd->regs_base->TX_FIFO_STATUS);
        *fillLvl_tx = (uint16_t)CPS_FLD_READ(MIPI__TX_FIFO_STATUS, TX_FIFO_FILL_LVL, reg_val);
        reg_val = CPS_REG_READ(&pd->regs_base->RX_FIFO_STATUS);
        *fillLvl_rx = (uint16_t)CPS_FLD_READ(MIPI__RX_FIFO_STATUS, RX_FIFO_FILL_LVL, reg_val);
        result = CDN_EOK;
    }
...
uint32_t I3C_SetSdrFifoFlush(I3C_PrivData* pd, bool flush_tx, bool flush_rx)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_SetSdrFifoFlushSF(pd) != CDN_EOK) {
        result = CDN_EINVAL;
    } else {
        reg_val = CPS_REG_READ(&pd->regs_base->FLUSH_CTRL);
        if (flush_tx) {
            reg_val = CPS_FLD_SET(MIPI__FLUSH_CTRL, TX_FLUSH, reg_val);
        }

        if (flush_rx) {
            reg_val = CPS_FLD_SET(MIPI__FLUSH_CTRL, RX_FLUSH, reg_val);
        }

        CPS_REG_WRITE(&pd->regs_base->FLUSH_CTRL, reg_val);
        result = CDN_EOK;
    }
...
/**
 * Read IBI pending progress status in Slave mode.
 * @param[in] pd Pointer to driver's private data object.
 * @param[in] pointer to tx fill level variable
 * @param[in] pointer to rx fill level variable
 * @return CDN_EOK on success
...
uint32_t I3C_SlaveGetIbiStatus(I3C_PrivData* pd, uint8_t* ibi_stat, uint8_t* hj_stat, uint8_t* mr_stat)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_SlaveGetIbiStatusSF(pd, ibi_stat, hj_stat, mr_stat) != CDN_EOK) {
        result = CDN_EINVAL;
    } else {
        reg_val = CPS_REG_READ(&pd->regs_base->SLV_STATUS2);
        *ibi_stat = (uint8_t)CPS_FLD_READ(MIPI__SLV_STATUS2, IBI_PEND, reg_val);
        *hj_stat = (uint8_t)CPS_FLD_READ(MIPI__SLV_STATUS2, HJ_PEND, reg_val);
        *mr_stat = (uint8_t)CPS_FLD_READ(MIPI__SLV_STATUS2, MR_PEND, reg_val);
        result = CDN_EOK;
    }
...
uint32_t I3C_GetAsfInfo(I3C_PrivData* pd, I3C_AsfInfo* asf_info)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_GetAsfInfoSF(local_pd, asf_info) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
...
uint32_t I3C_GetNewDevices(I3C_PrivData* pd, uint16_t* hj_devices)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_GetNewDevicesSF(pd, hj_devices) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
...
/**
 * Sends Command that sets NCA mode for private write in target mode.
 * @param[in] pd Pointer to driver's private data object.
 * @param[in] mode data pattern w or w/o subframing.
...
uint32_t I3C_CmdSetNCAMode(I3C_PrivData *pd, uint8_t mode, uint8_t devAddr, const I3C_TransmissionParameters *tx_params)
{
     ...
     * Validate parameters passed function
     */
    if (I3C_CmdSetNCAModeSF(pd, tx_params) != CDN_EOK) {
        DbgMsg(DBG_GEN_MSG, DBG_CRIT, "Error. Invalid arguments.\n");
        result = CDN_EINVAL;
    } else {
        ...
        if (cmd == NULL) {
            ...
            cmd->cmd |= i3c_cmd_ccc((uint64_t)I3C_CCC_SETTCTRL_DC);
            cmd->cmd |= i3c_cmd_dev_addr(devAddr);
            /* Size of SETTCTRL command payload is 1 */
            cmd->cmd |= i3c_cmd_get_payload_length(1U);
            cmd->payload_size = 1;
            cmd->payload_sdr = (volatile uint8_t *)&cmd->priv;
            cmd->payload_sdr[0] = (uint8_t)mode;
